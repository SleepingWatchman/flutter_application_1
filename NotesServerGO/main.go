...
func main() {
	...
	// Клиент ожидает /collaborators, а не /users
	...
	// Клиент ожидает /api/collaboration/sync/{database_id}
	...
	// Старый: router.HandleFunc("/health", controllers.HealthCheck).Methods(http.MethodGet)
	// apiRouter уже имеет префикс /api
	apiRouter.HandleFunc("/Service/status", controllers.HealthCheck).Methods(http.MethodGet)
